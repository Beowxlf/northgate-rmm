...
foreach($path in @($state,$install)){
 $current=$path
 while($current){if((Test-Path -LiteralPath $current) -and ((Get-Item -Force -LiteralPath $current).Attributes -band [IO.FileAttributes]::ReparsePoint)){throw 'Reparse installation path rejected'};$current=Split-Path $current}
}
...
if((Test-Path -LiteralPath $state) -or (Test-Path -LiteralPath $install)){
 ...
 if((Get-Item -Force -LiteralPath (Join-Path $install 'wulfgar.exe')).Attributes -band [IO.FileAttributes]::ReparsePoint){throw 'Reparse capture binary rejected'}
 if((Get-Item -Force -LiteralPath $configPath).Attributes -band [IO.FileAttributes]::ReparsePoint){throw 'Reparse configuration rejected'}
...
if(Test-Path -LiteralPath $stage){if((Get-Item -Force -LiteralPath $stage).Attributes -band [IO.FileAttributes]::ReparsePoint){throw 'Reparse installer directory rejected'}}else{New-Item -ItemType Directory -Path $stage|Out-Null}
function Get-VerifiedInstaller([string]$url,[string]$name,[string]$publisher){
 $path=Join-Path $stage $name
 if(Test-Path -LiteralPath $path){if((Get-Item -Force -LiteralPath $path).Attributes -band [IO.FileAttributes]::ReparsePoint){throw 'Reparse installer rejected'}}else{
