...
| Approved update metadata/status role     | Artifact repository update-status endpoint         | Approved publication protocol | G6                           | Signed status, allocator receipt, acks, and applicable rollout or restrictive authorization; repository independently verifies all          |
...
closed. The outer publication envelope carries the unchanged signed status
object, allocator receipt, and current intent and result acknowledgements. For an
authority-increasing transition it also carries the separately signed rollout
decision and applicable signed current-ring health or pre-deployment-test
attestation. For a freeze, revoke, or human-initiated pause it instead carries the
exact signed Z1 restrictive authorization. An automatic pause carries the exact
signed Z5 health-pause authorization. These objects remain beside, not inside, the
