...
| Audit writer                          | Control-plane VM                    | Append-oriented role distinct from normal readers and exporters                |
...
flowchart LR
  ...
  Session[Z8 Remote assistance] -. G7 only .-> Service
  Session -. Expiring single-target tunnel .-> Endpoints
...
| Z8 gateway       | Z4 exact endpoint        | Expiring tunnel only       | G7 conditional | One endpoint, protocol, port, and expiry; no subnet route  |
