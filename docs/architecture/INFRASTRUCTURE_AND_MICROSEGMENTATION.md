...
flowchart LR
  ...
  Admin -. Z5-unavailable-or-untrusted emergency evidence .-> Recovery
  ...
  Service -->|V1D synthetic; G2/G3 live issuance and status API| Issuer[Restricted endpoint issuer]
  ...
  Build -->|G6 sequence-free status request core| UpdateStatus[Separated update metadata/status authority]
  UpdateStatus -->|Allocate sequence and persist payload-bound receipt| SeqAllocator[External monotonic allocator]
  ...
  SeqRecovery -. Z5-unavailable-or-untrusted read exact fallback anchor .-> Recovery
  UpdateStatus -->|G6 signed status, allocation receipt, and rollout evidence| Artifacts
  UpdateStatus -->|G6 protected status intent and result acknowledgements| Observe
  UpdateStatus -->|G6 exact signed rollout-health attestation| Observe
  Observe -. G6 dedicated signed automatic-pause authorization .-> UpdateStatus
  UpdateStatus -. Z5-unavailable-or-untrusted intent and result acks .-> Recovery
  Build -->|G6 protected intent/result acknowledgements| Observe
  Artifacts -->|G6 append-only publication/revocation events| Observe
  Build -. Z5-unavailable-or-untrusted G6 intent/result acks .-> Recovery
  Artifacts -. Z5-unavailable-or-untrusted G6 audit fallback .-> Recovery
  ...
  Endpoints -. G6 fresh status and current activation head .-> Artifacts
  Endpoints -. G6 authority checkpoint after state uncertainty .-> UpdateStatus
  Endpoints -. G6 nonce-bound current allocator maximum .-> SeqAllocator
  Endpoints -. G6 nonce-bound current anchored maximum .-> Observe
  ...
  Admin -. G7 fallback exact-session/grant handle lookup .-> OSIdentity[Approved OS identity authority]
...
| Z2 control plane                         | Approved endpoint issuer                           | TCP 443                       | V1D synthetic; G2/G3 live    | Workload mTLS; V1D permits only exact-authorized synthetic issuance/revocation that is unusable by endpoints, with endpoint routes blocked; G2/G3 required for any endpoint-usable identity or endpoint traffic |
...
| Z1 security recovery operators           | Z6 immutable emergency-evidence intake             | Approved evidence protocol    | Z5 unavailable/untrusted     | Signed append-only fallback bundle; no restore/delete authority; reconcile only after Z5 re-establishment                                                                                                       |
...
| Z4 enrolled endpoint                     | Artifact repository and update-status endpoint     | TCP 443                       | G6                           | mTLS download authorization plus fresh status; recovery reads nonce-bound signed activation head/receipt                                                                                                        |
| Z4 enrolled endpoint                     | Approved update metadata/status authority          | TCP 443                       | G6 state recovery            | mTLS read-only nonce-bound authority checkpoint after uncertain state; no signing/admin authority                                                                                                               |
| Z4 enrolled endpoint                     | External allocator status endpoint                 | TCP 443                       | G6 state recovery            | Read-only nonce-bound signed current epoch/maximum; no allocation, reset, recovery, or policy authority                                                                                                         |
| Z4 enrolled endpoint                     | Z5 release-sequence proof endpoint                 | TCP 443                       | G6 state recovery            | Read-only nonce-bound signed current anchored maximum and continuity digest; no general audit read or write                                                                                                     |
...
| Approved update metadata/status role     | Artifact repository update-status endpoint         | Approved publication protocol | G6                           | Normal: status, allocator receipt, acks, rollout evidence. Emergency: restrictive status, pending-anchor receipt, Z1 authorization; verify all                                                                  |
| Z7 status coordinator                    | Approved update metadata/status authority          | TCP 443                       | G6                           | Sequence-free core; decision binds one-use correlation and predecessor; applicable signed health evidence                                                                                                       |
...
| Z5 dedicated health-pause authority      | Approved update metadata/status authority          | Approved pause protocol       | G6 automatic pause           | Failed threshold only; signed exact artifact/ring/policy/predecessor/correlation/expiry; pause only, no other status authority                                                                                  |
| Approved update metadata/status role     | Z5 protected release-audit intake                  | Approved TLS port             | G6                           | Append payload/allocator-receipt intent, then signed result; require an independent acknowledgement for each                                                                                                    |
| Approved update metadata/status role     | Z6 immutable release-audit fallback                | Approved evidence protocol    | G6, Z5 unavailable/untrusted | Same write-only intent/result and acknowledgements; no release, read, restore, or delete authority                                                                                                              |
...
| Z7 release services                      | Z6 immutable release-audit fallback                | Approved evidence protocol    | G6, Z5 unavailable/untrusted | Write-only signed event; independent acknowledgement before transition; no read/restore/delete authority                                                                                                        |
| Artifact repository audit emitter        | Z6 immutable release-audit fallback                | Approved evidence protocol    | G6, Z5 unavailable/untrusted | Write-only signed event; independent acknowledgement before transition; no read/restore/delete authority                                                                                                        |
...
| Z1 release recovery operator             | Update metadata/status emergency API               | TCP 443                       | G6 incident                  | Independent MFA; authorize higher-sequence freeze/revoke/pause status only; no release approval, resume, unfreeze, or artifact upload                                                                           |
...
| Z1 session recovery operator             | Approved OS identity authority recovery lookup     | Approved identity protocol    | G7 incident fallback         | Independent MFA and case scope; exact session plus grant query only when Z5 is unavailable, suspected, or untrusted; returns issuer/opaque handle only; no enumeration                                          |
...
record distinguishes claimed, observed, and verified outcome. When Z8 is
suspected, the independent Z1 network-recovery path blocks the exact
endpoint-facing policy-enforcement scope outside the gateway and verifies tunnel
teardown from endpoint or network evidence; it does not wait for or trust Z8.
...
recovery-audit intake. If Z5 is unavailable, suspected, or otherwise untrusted,
it writes the same signed bundle to the independent immutable Z6 evidence intake
with no read, restore, or delete authority. It reconciles the bundle only after
Z5 is re-established and independently verified. At least one trusted intake
...
Before returning any credential or handle, the OS identity authority durably and
atomically retains a signed, non-secret mapping from exact session/grant to
issuer, opaque revocation handle, credential status, and expiry for the
credential lifetime plus the approved incident-recovery retention window.
If Z5 is unavailable, suspected, or otherwise untrusted, the independent Z1
session-recovery identity may query only one exact session/grant pair and receive
only the issuer and opaque handle needed for revocation. The authority requires
independent MFA and case scope, prohibits listing, wildcard/time-range search,
credential-material access, issuance, renewal, and policy change, and records the
lookup and revocation through the trusted Z5 intake or Z6 fallback.
...
| Release-status sequence ledger and independent checkpoints                                              | The separated status authority anchors every allocated sequence and signed status digest in Z5 or Z6; when both are unavailable, only a Z1-authorized restrictive transition may use the external allocator's append-only pending anchor until reconciliation; allocator state remains outside the authority restore set | Z5/Z6 anchors plus external pending anchor | G6                                |
...
If Z5 is unavailable, suspected, or otherwise untrusted, the release service or
repository sends the same signed, sequence- and correlation-bound event through
its write-only identity to the immutable Z6 release-audit fallback and requires
that independent acknowledgement instead. Reconciliation waits until Z5 is
re-established and independently verified.

The same invariant applies to signed release-status transitions. Z7 first
canonicalizes a sequence-free status request core containing the exact action,
artifact and metadata digests, version, current/destination rings, policy,
single-use correlation ID, predecessor signed-status digest, expiry, applicable
rollout-decision, Z1 restrictive-authorization, or Z5 health-pause-authorization
digest, and health-attestation digest. After verifying the request evidence, the
metadata/status authority submits the unchanged core and its digest to the
external monotonic allocator. The allocator validates the authority epoch,
atomically chooses the next sequence, materializes the canonical
acknowledgement-free status payload by adding that epoch and sequence, computes
the payload digest, and durably persists a signed allocation receipt binding the
epoch, sequence, request-core digest, payload digest, action, correlation, and
predecessor. The receipt and completed payload are returned together. This
construction is acyclic: neither object contains its own digest.

Before issuing an intent acknowledgement, Z5 or Z6 pins the allocator key and
independently verifies the receipt signature and every binding against the exact
payload. A separate status envelope carries the unchanged payload, declared
payload digest, allocator receipt, and intent acknowledgement; the envelope is
not hashed into its own payload digest. The separated metadata/status authority
pins the allocator and Z5/Z6 keys, recomputes the core and payload digests, and
rejects a missing, invalid, expired, rolled-back, wrong-epoch, wrong-action,
wrong-ring, wrong-sequence, wrong-correlation, wrong-predecessor, or
wrong-payload-digest receipt or acknowledgement. After signing the status object,
...
closed. A normal outer publication envelope carries the unchanged signed status
object, allocator receipt, and current intent and result acknowledgements. An
authority-increasing transition also carries the separately signed rollout
decision and applicable signed current-ring health or pre-deployment-test
attestation. A human-initiated freeze, revoke, or pause carries the exact signed
Z1 restrictive authorization. An automatic pause carries the exact signed Z5
health-pause authorization through this normal acknowledged path.

When both protected intakes are unavailable or untrusted, the emergency outer
publication envelope carries no audit acknowledgements. It contains only the
restrictive signed status, the allocator's unreconciled signed pending-anchor
receipt, and the exact signed Z1 freeze/revoke/pause authorization. These objects
remain beside, not inside, the
acknowledgement-free status payload. The receipt binds the payload digest, while
the payload binds the applicable authorization and attestation digests; none is
hashed into itself. The repository pins the approval, attestation, Z1
recovery-authorization, and Z5 health-pause keys and independently verifies their
signatures, roles, separation of duties, exact artifact and metadata digests,
version, action, source and destination rings, policy, window, thresholds,
outcome, freshness, status-bound digests, and the separately pinned allocator
receipt before activation. It MUST NOT trust the metadata/status authority's
assertion that those checks passed. The repository verifies a restrictive
authorization's exact action, artifact/rollout scope, predecessor, correlation,
not-before, and expiry and MUST NOT interpret it as authority to start, advance,
resume, or unfreeze a rollout.

Automatic pause authority is separate from rollout approval and human recovery.
When a defined availability or security threshold fails, the dedicated Z5
health-pause role signs a short-lived authorization binding only `pause`, the
exact failed health attestation and evidence digest, artifact, current ring,
policy, predecessor status, single-use correlation, and expiry. The status
authority and repository pin that key and verify the failing result and exact
bindings. The role cannot freeze, revoke, start, advance, resume, unfreeze, change
policy/thresholds, or issue a healthy attestation. If the health evaluator is
unavailable or untrusted, this automated path fails closed and the independent Z1
...
fresh challenge nonce and directly retrieves four independently signed responses:
the authority checkpoint; the allocator's current epoch and maximum; Z5's current
anchored maximum and continuity digest; and the artifact repository's current
activation head and activation receipt. Every response binds that nonce, the
authority epoch, sequence, status-payload digest, and a maximum 60-second expiry.
The activation receipt also binds the publication-envelope digest and activation
time. The endpoint pins all four keys, verifies signatures and exact bindings,
requires the allocator and anchored maxima plus the activation head to agree, and
only then atomically raises its local floor. A gap, stale response, unavailable or
untrusted source, non-current activation, mismatch, or rollback blocks
installation until reconciliation produces a new agreed active head. The status
authority or artifact repository cannot initialize or lower the floor by itself.
...
to the exact acknowledgement-free status-payload digest by a signed allocator
receipt that is independently verified by the protected intake, repository, and
checkpoint consumer, then written to the protected Z5 sequence ledger or the
...
- the Z1 session-recovery identity cannot issue, extend, inspect, or change the
  ...
  only the exact session/grant's opaque revocation handle and issuer from Z5, or
  from the OS identity authority when Z5 is unavailable, suspected, or untrusted;
  neither route permits enumeration, wildcard, or bounded-time searches;
...
- the Z5 health-pause authority cannot authorize freeze, revoke, start, advance,
  resume, unfreeze, a healthy threshold result, or a pause outside the exact
  failed artifact/ring/policy/predecessor/correlation scope;
...
- agent restart, reinstall, update rollback, VM/OS restore, or local sequence-state
  ...
  authority, allocator, Z5 ledger, or artifact repository alone to reinitialize
  it; missing or disagreeing current proofs must fail closed;
- a G6 publisher or repository cannot complete an authority-increasing signing,
  ...
  acknowledgement from the independent Z5 protected intake or, while Z5 is
  unavailable or untrusted, the immutable Z6 fallback; its own local buffer never
  qualifies;
...
- a G6 emergency freeze/revocation test with the artifact service suspected,
  ...
  reach a trusted intake and use Z6 when Z5 is unavailable or suspected;
- a G7 emergency-termination test with Z2 unavailable/suspected, proving Z1 intent
  ...
  Z5 is unavailable or suspected;
...
- a G7 suspected-gateway active-session test proving the independent Z1 network
  recovery path blocks the exact Z8 endpoint-facing enforcement scope, verifies
  stream and tunnel teardown from endpoint/network evidence, and revokes the JIT
  credential without trusting a gateway termination response;
...
- a G7 compound-failure test that treats Z5 and Z8/broker as suspected, retrieves
  only the exact session/grant handle through the OS identity authority fallback,
  records the lookup and revocation in Z6, and proves enumeration, cross-session
  lookup, issuance, and policy change remain denied;
...
- G6 tests proving successful and denied signing, publication, freeze, and
  ...
  then proving Z5 loss or suspected compromise requires an independent Z6
  acknowledgement and later verified Z5 reconciliation; loss of both trusted
  intakes must block authority-increasing work but
...
- G6 allocator-receipt tests treating the metadata/status authority as
  compromised and proving Z5/Z6 intake, the repository, and a checkpoint consumer
  reject missing, forged, replayed, rolled-back, or wrong-epoch/sequence/core/
  payload/action/correlation/predecessor receipts even when the status signature
  and audit acknowledgements are otherwise valid;
...
- G6 automatic-pause tests proving a signed failed threshold can invoke only the
  dedicated pause action for its exact artifact, ring, policy, predecessor, and
  correlation, while forged/healthy/stale/mismatched evidence and every
  freeze/revoke/start/advance/resume/unfreeze request are rejected;
...
- a G6 repository-boundary test treating the metadata/status authority as
  compromised and proving an otherwise valid signed status and acknowledgement
  chain is rejected when the carried decision or attestation is missing, forged,
  stale, self-approved, signed by the wrong role, bound to the wrong artifact,
  ring, action, window, threshold, result, or policy, or mismatched to the digest
  in the status payload;
- a G6 restrictive-status repository test rejecting missing, forged, stale,
  wrong-role/action/scope/predecessor/correlation, or digest-mismatched signed Z1
  authorization and proving it cannot authorize start, advance, resume, or
  unfreeze;
...
- a G6 rollback-state test that restarts, reinstalls, rolls back, and restores the
  ...
  status is rejected; uncertain state must require fresh nonce-bound agreement
  among the authority checkpoint, allocator maximum, Z5 anchored maximum and
  continuity proof, and repository activation head/receipt. Any stale,
  unavailable, non-current, or disagreeing proof must block installation;
