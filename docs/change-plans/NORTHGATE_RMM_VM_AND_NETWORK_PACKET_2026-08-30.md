...
| RMM-NET-016 | Z5 audit-archive exporter         | `TBD protected Z6 backup target`                    | `TBD approved backup protocol` | Required before G2; audit/checkpoint scope, write only                                                                                      | Security evidence owner | `NG-CHG-20260830-001` |
| RMM-NET-017 | Z6 backup/recovery monitor        | `TBD protected Z5 telemetry sink`                   | `TBD approved TLS port`        | Write-only health; no backup or recovery content                                                                                            | Recovery owner          | `NG-CHG-20260830-001` |
| RMM-NET-018 | Z6 source-record collector        | `TBD exact RMM source/deployment repository alias`  | TCP 443                        | Read-only exact commit, signed tag, and manifest scope                                                                                      | Recovery owner          | `NG-CHG-20260830-001` |
| RMM-NET-019 | Z5 telemetry service              | `TBD independent alert destination`                 | `TBD approved TLS port`        | Required before G2; notification-only credential and redacted bounded payload                                                               | Security monitoring     | `NG-CHG-20260830-001` |
| RMM-NET-020 | Z1 endpoint-PKI recovery operator | `TBD endpoint issuer emergency revocation endpoint` | TCP 443                        | Required before G2; independent MFA; exact endpoint identity; revoke/status only; signed intent/result; terminate bound sessions or isolate | Endpoint PKI owner      | `NG-CHG-20260830-001` |
