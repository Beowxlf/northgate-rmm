...
| RMM-NET-018 | Z6 source-record collector     | exact RMM source/deployment repository | TCP 443                        | Read-only exact commit, signed tag, and manifest scope     | Recovery owner          | `NG-CHG-20260830-001` |
| RMM-NET-019 | Z5 telemetry service           | `TBD independent alert destination`    | `TBD approved TLS port`        | Notification-only credential and redacted bounded payload  | Security monitoring     | `NG-CHG-20260830-001` |
