...
| TM-38 | Compromised coordinator bypasses rollout health gates | approval binds one-use correlation and predecessor; repository verifies and consumes it           | reject status, isolate coordinator, freeze rollout, preserve evidence     |
...
| TM-40 | Status-authority restore reissues a lower sequence    | max-floor reconciliation; allocator-only restrictive recovery when both anchors are unavailable   | keep general signing disabled, recover under dual control, preserve chain |
