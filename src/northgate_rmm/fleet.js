...
(() => {
  ...
  async function refresh(manual = false) {
    ...
    try {
      ...
      $("refresh-state").textContent = `Updated ${new Date(state.lastRefresh).toLocaleTimeString()}`;
      ...
      $("connection-banner").textContent =
        error.name === "AbortError"
          ...
          : `${error.message} Displayed data is from ${state.lastRefresh ? new Date(state.lastRefresh).toLocaleString() : "an unavailable snapshot"}.`;
  ...
  function openDevice(id, tab = "overview") {
    ...
    const tabs = [
      ...
      ...(d.permissions.includes("remote")
        ? [
            ["workspace", "SSH & files"],
            ...((d.remote_methods ? d.remote_methods.includes("rdp") : d.platform === "windows") ? [["desktop", "Desktop"]] : []),
    ...
    function select(key) {
      for (const [name, label] of tabs) {
        ...
        if (selected && name === "desktop" && !pane.dataset.desktopMounted) {
          pane.dataset.desktopMounted = "true";
          pane.innerHTML = `<section class="panel"><div class="panel-header"><div><h2>Remote Desktop</h2><p>Open this device in Windows Remote Desktop, then move the window to any monitor.</p></div></div><div class="panel-body"><a class="button primary" href="/remote/${h(d.id)}/desktop.rdp">Download RDP connection</a><p class="subtle">Open the downloaded .rdp file on Smooth Operator. Enrolled owner profiles include a Windows-encrypted password that only that Windows user can decrypt. Other users receive a connection file and can use Access &amp; secrets if prompted.</p><p class="subtle">Direct connections require the permitted lab workstation. Desktop sign-in uses the machine account; RMM browser session controls do not govern the native connection.</p><details><summary>Use browser desktop instead</summary><div class="browser-desktop"></div></details></div></section>`;
          pane.querySelector("details").addEventListener("toggle", (event) => {
            const container = pane.querySelector(".browser-desktop");
            if (event.target.open && !container.querySelector("iframe")) {
              const iframe = document.createElement("iframe");
              iframe.title = "Browser desktop — " + d.name;
              iframe.src = `/remote/${d.id}/desktop`;
              bindToolTheme(iframe);
              container.append(iframe);
            }
          });
        }
        if (selected && !["overview", "inspect", "workspace", "tool-catalog", "desktop"].includes(name) && !pane.querySelector("iframe")) {
          const iframe = document.createElement("iframe");
