...
for(const action of Object.keys(settings.actions)){if((action==="bitlocker.escrow"&&settings.platform!=="windows")||(["bitlocker.escrow","recovery.rotate"].includes(action)&&!settings.recovery))continue;const option=document.createElement("option");option.value=action;option.textContent=names[action]||action;$("operation").append(option);}
function form(){const action=$("operation").value;$("parameters").replaceChildren();for(const key of settings.actions[action]){if(action==="files.write"&&["data","sha256"].includes(key))continue;const label=document.createElement("label");label.textContent=key.replaceAll("_"," ")+" ";const input=document.createElement(key==="inputs"?"textarea":"input");input.name=key;if(numbers.has(key))input.type="number";else if(key==="overwrite")input.type="checkbox";input.value=defaults[key]??"";label.append(input);$("parameters").append(label,document.createElement("br"));}if(action==="files.write"){const file=document.createElement("input");file.type="file";file.name="upload";$("parameters").append(file);}}
