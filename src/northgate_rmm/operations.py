...
class Operations:
        ...
        if isinstance(p, IntegrationActor):
            ...
            try:
                ...
                if permission not in entry["actions"] or not endpoints:
                    return False
                if p.snapshot_endpoints is not None:
                    return all(
                        endpoint in p.snapshot_endpoints for endpoint in endpoints
                    )
    ...
    def snapshot(self, p: Any) -> Any:
        if not isinstance(p, IntegrationActor):
            return self._snapshot(p)
        native = self.m.integration
        if native is None:
            raise web.HTTPForbidden()
        entry = native.auth.current(p.entry)
        if "ops.view" not in entry["actions"]:
            raise web.HTTPForbidden()
        admitted = set()
        for endpoint in entry["endpoints"]:
            try:
                native.endpoint(entry, endpoint, permission="ops.view")
            except (ValueError, KeyError, NotFoundError, web.HTTPException):
                continue
            admitted.add(endpoint)
        # Request-local only: avoid reopening monitoring DB connections for every
        # nested record. Revalidate all admitted enrollments before releasing data.
        result = self._snapshot(
            replace(p, entry=entry, snapshot_endpoints=frozenset(admitted))
        )
        for endpoint in admitted:
            native.endpoint(entry, endpoint, permission="ops.view")
        if native.auth.current(entry) != entry:
            raise web.HTTPForbidden(text="Integration scope changed during read")
        return result

    def _snapshot(self, p: Any) -> Any:
        result: dict[str, Any] = {}
        plural = {
            "case": "cases",
            "asset": "assets",
