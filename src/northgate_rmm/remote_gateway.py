...
from northgate_rmm.native_desktop import NativeDesktopProfiles
...
class RemoteGateway:
    def __init__(
        ...
        *,
        ...
        self.native_secret_resolver: (
            Callable[
                [web.Request, OperatorPrincipal, RemoteTarget],
    ...
    async def desktop_file(self, request: web.Request) -> web.Response:
        ...
        target, parameters = self.method_target(endpoint_id, "rdp")
        ...
        if self.native_profiles:
            try:
                profile = self.native_profiles.record(principal.subject, target)
                if profile:
                    if not self.native_secret_resolver:
                        raise web.HTTPServiceUnavailable()
                    fields = await self.native_secret_resolver(
                        request, principal, target
                    )
                    encrypted = self.native_profiles.validate(profile, target, fields)
                    username = profile["username"]
                    password_lines = ["password 51:b:" + encrypted]
            except (ValueError, KeyError, OSError):
                raise web.HTTPConflict(
                    text="Saved workstation credentials need refreshing in the RMM."
                ) from None
        await self.audit(
        ...
        content = "\r\n".join(
            [
                ...
                *password_lines,
                ...
                + ("0" if rdp_security(parameters) == "tls" else "1"),
    ...
    async def _landing(self, request: web.Request) -> web.Response:
        ...
        if method == "rdp":
            ...
            try:
                rdp_security(options)
            except ValueError:
                raise web.HTTPServiceUnavailable(
                    text="RDP security configuration is invalid"
                ) from None
