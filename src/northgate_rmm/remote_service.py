...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=os.environ.get("NORTHGATE_RMM_NATIVE_DESKTOP_PROFILES"),
    )
    ...
    if args.native_desktop_profiles:
        from northgate_rmm.native_desktop import NativeDesktopProfiles

        gateway.native_profiles = NativeDesktopProfiles(
            args.native_desktop_profiles, key
        )
    credentials = {}
    if args.credentials:
        with regular_file_reference(
