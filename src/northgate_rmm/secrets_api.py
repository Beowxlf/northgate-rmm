...
class SecretsAPI:
    def __init__(
        ...
        *,
        ...
        gateway.native_secret_resolver = self.resolve_native_export
    ...
    async def resolve_native_export(self, request, principal, target):
        ...
        if not fields.get("__rmm_secret_id"):
            raise web.HTTPConflict(text="Select a saved RDP credential first")
        await self.authorize(request, target.endpoint_id, "reveal")
        await self.audit(
            principal,
            target.endpoint_id,
            "native_encrypted_export",
            fields["__rmm_secret_id"],
        )
        return fields
